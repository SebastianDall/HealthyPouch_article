...
# Download the three files into the data folder.
# wget https://data.gtdb.aau.ecogenomic.org/releases/release226/226.0/bac120_metadata_r226.tsv.gz
# wget https://data.gtdb.aau.ecogenomic.org/releases/release226/226.0/ar53_metadata_r226.tsv.gz
# wget https://data.gtdb.aau.ecogenomic.org/releases/release226/226.0/genomic_files_all/ssu_all_r226.fna.gz

# Unzip all files and run the following cmd once:
# grep ">" ssu_all_r226.fna | sed 's/>//' | sed 's/~/\t/' | sed 's/ /\t/' > ssu_all_r226_ref_clean.tsv
...
# papr88 <- fread("HealthyPouch_article/data/mmlong2_lite/NP-PaPr00000088MP/tmp/binning/round_1/checkm2/quality_report.tsv") %>% 
#   mutate(sample = "NP-PaPr00000088MP")
#
# papr88_cov <- fread("HealthyPouch_article/data/mmlong2_lite/NP-PaPr00000088MP/tmp/binning/round_1/cov.tsv") %>% 
...
#
#
# papr11 <- fread("HealthyPouch_article/data/mmlong2_lite/NP-PaPr00000011MP/tmp/binning/round_1/checkm2/quality_report.tsv") %>% 
#   mutate(sample = "NP-PaPr00000011MP")
#
# papr11_cov <- fread("HealthyPouch_article/data/mmlong2_lite/NP-PaPr00000011MP/tmp/binning/round_1/cov.tsv") %>% 
#   rename(contig = contigname) %>% 
...
#   mutate(
#     quality_report = map(.x = sample, ~fread(paste0("HealthyPouch_article/data/mmlong2_lite/",.x, "/results/", .x, "_bins.tsv")))
#   )
#
# checkm <- checkm %>% 
...
mimag <- fread("./data/mimag_bin_quality.tsv")
...
# gtdb_mag_tax <- tibble(
#   batch = list.dirs("HealthyPouch_article/output/mmlong2_lite_gtdb/", recursive = F, full.names = T)
# ) %>% 
#   mutate(
...
#write_delim(gtdb_mimag_stats_distinct, file = "data/genome_stats.tsv", delim ="\t")
...
#   filter(species == "s__") %>% 
#   select(bin) %>% 
#   mutate(file = paste0("HealthyPouch_article/data/mmlong2_lite_collected_bins/",bin,".fa")) %>% 
#   relocate(file) %>% 
...
#
# drep <- tibble(
#   sample = list.files("micropouch/subprojects/5.data_upload/output/drep")
# ) %>% 
#   mutate(
#     bin = map(.x = sample, ~(list.files(file.path("micropouch/subprojects/5.data_upload/output/drep", .x, "dereplicated_genomes"))))
#   ) %>% 
#   unnest(bin) %>% 
